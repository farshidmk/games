  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        <fieldset className="w-112">
          <div className="relative border border-gray-300 text-gray-800 bg-white shadow-lg">
            ...
            <Controller
              ...
              render={({ field }) => {
                return (
                  <select className="appearance-none w-full py-1 px-2 bg-white" id="frm-whatever" {...field}>
        ...
        {
          <div>
            {[...Array(Number(watch('numberOfPlayers'))).keys()].map((index) => {
              return (
                <input
                  {...register(`players.${index - 1}` as keyof ChoosePlayersFormType)}
